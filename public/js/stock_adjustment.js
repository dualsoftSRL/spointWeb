$(document).ready(function() {
    ...
    if ($('#search_product_for_srock_adjustment').length > 0) {
        ...
        $('#search_product_for_srock_adjustment')
                ...
                select: function(event, ui) {
                    if (ui.item.qty_available > 0) {
                        ...
                        if($("#adjustment_type").val()==''){
                            alert("Selecciona un tipo de ajuste")
                        }else{
                        stock_adjustment_product_row(ui.item.variation_id);
                        }
                    } else {
                        if (ui.item.qty_available <=0 && $("#adjustment_type").val()=='normal'){
                            $(this).val(null);
            ...
            if (item.qty_available <= 0 && $("#adjustment_type").val()=='abnormal') {
...
function stock_adjustment_product_row(variation_id) {
    ...
    var type_adjustment=$("#adjustment_type").val();
    $.ajax({
        method: 'POST',
        url: '/stock-adjustments/get_product_row',
        data: { row_index: row_index, variation_id: variation_id, location_id: location_id ,type_adjustment:type_adjustment},
        dataType: 'html',
        success: function(result) {
