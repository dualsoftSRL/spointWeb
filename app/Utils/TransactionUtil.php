...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'idsecuencia' => $input["idsecuencia"],
            'vencencf' => $input["vencencf"]
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output['rnc']=$business_details->rnc;
        $output['vencencf']=$transaction->vencencf;
        //Display name
        ...
        //Logo
        $output['logo'] = $il->show_logo != 0 && !empty($il->logo) && file_exists(public_path('uploads/invoice_logos/' . $il->logo)) ? asset('public/uploads/invoice_logos/' . $il->logo) : false;
        ...
        foreach ($lines as $line) {
            ...
            if ($il->show_image == 1) {
                ...
                if (count($media)) {
                    ...
                    $line_array['image'] = !empty($first_img->display_url) ? $first_img->display_url : asset('public/img/default.png');
                } else {
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            $query = Transaction::join('purchase_lines AS PL', 'transactions.id', '=', 'PL.transaction_id')
                ->leftjoin('variation_location_details as vld','vld.product_id','=','PL.product_id')
            ...
            $rows = $query->select(
                ...
                'vld.qty_available as cantidadtotal',
            ...
            foreach ($rows as $k => $row) {
                ...
                if ($qty_selling <= $row->cantidadtotal) {
                    $qty_allocated = $qty_selling;
                    $qty_selling = 0;
                } else {
                    $qty_selling = $qty_selling - $row->cantidadtotal;
                    $qty_allocated = $row->cantidadtotal;
                }
