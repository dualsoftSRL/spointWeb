...
public function getCXC(Request $request)
{
    ...
    $da = \Carbon::now();
    $da1=$da->format('Y-m-d');
$da15=$da->modify('-15 days')->format('Y-m-d');
$da30=$da->modify('-30 days')->format('Y-m-d');
$da45=$da->modify('-45 days')->format('Y-m-d');
$da60=$da->modify('-60 days')->format('Y-m-d');
    ...
    $contacts = Contact::where('contacts.business_id', $business_id)
        ...
        ->select(
            DB::raw("SUM(IF(t.type = 'purchase', final_total, 0)) as total_purchase"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final'  AND  t.transaction_date>='$da15' AND t.transaction_date<='$da1', final_total, 0)) as total_0_15"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.transaction_date>='$da15' AND t.transaction_date<='$da1', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as opening_balance_paid_0_15"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND t.transaction_date>='$da15' AND t.transaction_date<='$da1', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as total_paid_0_15"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.status = 'final' AND t.transaction_date>='$da15' AND t.transaction_date<='$da1', final_total, 0)) as opening_0_15"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND t.transaction_date>='$da30' AND t.transaction_date<='$da15', final_total, 0)) as total_15_30"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.transaction_date>='$da30' AND t.transaction_date<='$da15', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as opening_balance_paid_15_30"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.status = 'final' AND t.transaction_date>='$da30' AND t.transaction_date<='$da15', final_total, 0)) as opening_15_30"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND t.transaction_date>='$da30' AND t.transaction_date<='$da15', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as total_paid_15_30"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND t.transaction_date>='$da45' AND t.transaction_date<='$da30', final_total, 0)) as total_30_45"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.status = 'final' AND t.transaction_date>='$da45' AND t.transaction_date<='$da30', final_total, 0)) as opening_30_45"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.transaction_date>='$da45' AND t.transaction_date<='$da30', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as opening_balance_paid_30_45"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND t.transaction_date>='$da45' AND t.transaction_date<='$da30', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as total_paid_30_45"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND t.transaction_date>'$da60' AND t.transaction_date<='$da45', final_total, 0)) as total_45_60"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.transaction_date>'$da60' AND t.transaction_date<='$da45', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as opening_balance_paid_45_60"),

            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.status = 'final' AND t.transaction_date>'$da60' AND t.transaction_date<='$da45', final_total, 0)) as opening_45_60"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND t.transaction_date>'$da60' AND t.transaction_date<='$da45', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as total_paid_45_60"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND transaction_date<'$da60', final_total, 0)) as total_dias_mas_60"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.transaction_date<'$da60', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as opening_balance_paid_mas_60"),
            DB::raw("SUM(IF(t.type = 'opening_balance' AND t.status = 'final' AND transaction_date<'$da60', final_total, 0)) as opening_dias_mas_60"),
            DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND transaction_date<'$da60', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as total_paid_mas_60"),
        ...
        return Datatables::of($contacts)
            ->editColumn('name', function ($row) {
                ...
                return '<a href="' . action('ContactController@showdetailcontact', [$row->id]) . '" target="_blank" class="no-print showdetail_contact_button">' .
                        $name .
                    '</a><span class="print_section">' . $name . '</span>';
            })
            ->editColumn('total_45_60', function ($row) {
                $total_45_60=($row->total_45_60-$row->total_paid_45_60)+($row->opening_45_60-$row->opening_balance_paid_45_60);;
                return '<span class="display_currency total_45_60" data-orig-value="' . $total_45_60 . '" data-currency_symbol = true>' . $total_45_60 . '</span>';
            })
            ->editColumn('total_0_15', function ($row) {
                $total_0_15=($row->total_0_15-$row->total_paid_0_15)+($row->opening_0_15-$row->opening_balance_paid_0_15);

                return '<span class="display_currency total_0_15" data-orig-value="' . $total_0_15. '" data-currency_symbol = true>' . $total_0_15 . '</span>';
            })
            ->editColumn('total_30_45', function ($row) {
                $total_30_45=($row->total_30_45-$row->total_paid_30_45)+($row->opening_30_45-$row->opening_balance_paid_30_45);
                return '<span class="display_currency total_30_45" data-orig-value="' . $total_30_45 . '" data-currency_symbol = true>' . $total_30_45 . '</span>';
            })
            ->editColumn('total_15_30', function ($row) {
                $total_15_30=($row->total_15_30-$row->total_paid_15_30)+($row->opening_15_30-$row->opening_balance_paid_15_30);
                return '<span class="display_currency total_15_30" data-orig-value="' . $total_15_30 . '" data-currency_symbol = true>' . $total_15_30 . '</span>';
            })
            ->addColumn('due', function ($row) {
                $due = ($row->total_invoice - $row->invoice_received - $row->total_sell_return + $row->sell_return_paid) - ($row->total_purchase - $row->total_purchase_return + $row->purchase_return_received - $row->purchase_paid) + ($row->opening_balance - $row->opening_balance_paid);
            ...
            ->editColumn('total_dias_mas_60', function ($row) {
                $total_dias_mas_60=($row->total_dias_mas_60-$row->total_paid_mas_60)+($row->opening_dias_mas_60-$row->opening_balance_paid_mas_60);
                return '<span class="display_currency total_dias_mas_60" data-orig-value="' . $total_dias_mas_60 . '" data-currency_symbol = true>' . $total_dias_mas_60 . '</span>';
            })
            ->removeColumn('supplier_business_name')
            ->removeColumn('invoice_received')
            ->removeColumn('purchase_paid')
            ->removeColumn('id')
            ->rawColumns(['total_45_60', 'total_15_30', 'due', 'name', 'total_0_15', 'total_30_45', 'total_dias_mas_60'])
            ->make(true);
    }
    ...
    return view('report.contact_cxc')
    ->with(compact('customer_group', 'types'));
}

public function getCXP(Request $request)
{
    if (!auth()->user()->can('contacts_report.view')) {
        abort(403, 'Unauthorized action.');
    }
...
$da60=$da->modify('-60 days')->format('Y-m-d');
    ...
    $contacts = Contact::where('contacts.business_id', $business_id)
        ...
        return Datatables::of($contacts)
            ...
            ->editColumn('total_45_60', function ($row) {
                $total_45_60=($row->total_45_60+$row->opening_45_60);
                return '<span class="display_currency total_45_60" data-orig-value="' . $total_45_60 . '" data-currency_symbol = true>' . $total_45_60 . '</span>';
            })
            ->editColumn('total_0_15', function ($row) {
                $total_0_15=($row->total_0_15+$row->opening_0_15);

                return '<span class="display_currency total_0_15" data-orig-value="' . $total_0_15. '" data-currency_symbol = true>' . $total_0_15 . '</span>';
            })
            ->editColumn('total_30_45', function ($row) {
                $total_30_45=($row->total_30_45+$row->opening_30_45);
                return '<span class="display_currency total_30_45" data-orig-value="' . $total_30_45 . '" data-currency_symbol = true>' . $total_30_45 . '</span>';
            })
            ->editColumn('total_15_30', function ($row) {
                $total_15_30=($row->total_15_30+$row->opening_15_30);
                return '<span class="display_currency total_15_30" data-orig-value="' . $total_15_30 . '" data-currency_symbol = true>' . $total_15_30 . '</span>';
            })
            ->addColumn('due', function ($row) {
                $due = ($row->total_invoice - $row->invoice_received - $row->total_sell_return + $row->sell_return_paid) - ($row->total_purchase - $row->total_purchase_return + $row->purchase_return_received - $row->purchase_paid) + ($row->opening_balance - $row->opening_balance_paid);
            ...
            ->editColumn('total_dias_mas_60', function ($row) {
                $total_dias_mas_60=($row->total_dias_mas_60+$row->opening_dias_mas_60);
                return '<span class="display_currency total_dias_mas_60" data-orig-value="' . $total_dias_mas_60 . '" data-currency_symbol = true>' . $total_dias_mas_60 . '</span>';
            })
            ->removeColumn('supplier_business_name')
            ->removeColumn('invoice_received')
    ...
    return view('report.proveedores_cxc')
    ->with(compact('customer_group', 'types'));
}
