...
<section class="content no-print">
    @component('components.filters', ['title' => __('report.filters')])
        @include('sell.partials.sell_list_filters_cxc')
        @if($is_woocommerce)
            <div class="col-md-4">
    ...
    @component('components.widget', ['class' => 'box-primary', 'title' => __( 'lang_v1.all_sales')])
        ...
        @if(auth()->user()->can('direct_sell.access') ||  auth()->user()->can('view_own_sell_only'))
            ...
            <button onclick="probar()" id="pagartodos" class="btn btn-success" style="margin-bottom:10px;">Pagar las seleccionadas</button>
...
var contactoid=<?php echo $contact_id ?>;
...
function probar(){
    ...
    $("input:checkbox:checked").each(function() {
            ...
            selected.push(idseleccionado);
            console.log(idseleccionado);
            var url="{{ action('TransactionPaymentController@Pagar', ":id") }}";
...
$("#sell_list_filter_customer_id").val(contactoid);
var texto=$('select[name="sell_list_filter_customer_id"] option:selected').text();
$('input[name="sell_list_customer"]').val(texto);
// $("#sell_list_customer").text(texto);
    ...
    sell_table = $('#sell_table').DataTable({
        ...
        createdRow: function( row, data, dataIndex ) {
            ...
            $( row ).find('td:eq(0)').click(function(e){
                e.stopPropagation();
            });
        }
    });
