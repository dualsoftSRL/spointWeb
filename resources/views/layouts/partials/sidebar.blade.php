    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li class="{{in_array($request->segment(1), ['purchases', 'purchase-return']) ? 'active active-sub' : '' }}" id="tour_step6">
          <a href="{{action('PurchaseController@index')}}" id="tour_step6_menu"><i class="fa fa-arrow-circle-down"></i> <span>@lang('purchase.purchases')</span>
        ...
        @if(auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update') )
        <li class="{{in_array($request->segment(1), ['purchases', 'purchase-return']) ? 'active active-sub' : '' }}" id="tour_step6">
          <a href="{{action('ProjectController@index')}}" id="tour_step6_menu"><i class="fa fa-arrow-circle-down"></i> <span>Proyectos</span>
        ...
        <li>
          <a href="#"><i class="fa fa-database" aria-hidden="true"></i> <span>@lang('lang_v1.inventary')</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
        ...
        <li class=" {{  in_array( $request->segment(1), ['expense-categories', 'expenses']) ? 'active active-sub' : '' }}">
          <a href="{{action('ExpenseController@index')}}"><i class="fa fa-minus-circle"></i> <span>@lang('expense.expenses')</span>
          ...
          <li class="treeview {{  in_array( $request->segment(1), ['reports']) ? 'active active-sub' : '' }}" id="tour_step8">
            ...
            <ul class="treeview-menu">
              ...
              @can('contacts_report.view')
                ...
                <li class="{{ $request->segment(2) == 'customer-supplier' ? 'active' : '' }}" ><a href="{{action('ReportController@getCXC')}}"><i class="fa fa-address-book"></i>@lang('report.contacts_cxc')</a></li>
                <li class="{{ $request->segment(2) == 'customer-supplier' ? 'active' : '' }}" ><a href="{{action('ReportController@getCXP')}}"><i class="fa fa-address-book"></i>@lang('report.contacts_cxp')</a></li>
        ...
        <li class="treeview @if( in_array($request->segment(1), ['business', 'tax-rates', 'barcodes', 'invoice-schemes', 'business-location', 'invoice-layouts', 'printers', 'subscription', 'types-of-service']) || in_array($request->segment(2), ['tables', 'modifiers']) ) {{'active active-sub'}} @endif">
          ...
          <ul class="treeview-menu" id="tour_step3">
            @can('business_settings.access')
              ...
              <!-- <li class="{{ $request->segment(1) == 'ncf' ? 'active' : '' }}" >
                <a href="{{action('NcfController@index')}}"><i class="fa fa-file"></i> @lang('business.ncf')</a>
              </li> -->
            @endcan
            @can('invoice_settings.access')
              <li class="@if( in_array($request->segment(1), ['invoice-schemes', 'invoice-layouts']) ) {{'active'}} @endif">
                <a href="{{action('InvoiceSchemeController@index')}}"><i class="fa fa-cog"></i> <span>@lang('invoice.invoice_settings')</span></a>
              </li>
            @endcan
